...
let observer = {
  ...
  _applyToWindow: function(window, document)
  {
    ...
    button.addEventListener("command", this.toggleEncryption.bind(this, label, button, uri));

    rows.appendChild(row);
    insertedElements.set(window, row);
  },

  toggleEncryption: function(label, button, uri)
  {
    let isSecure = this.service.isSecureURI(this.service.HEADER_HSTS, uri, 0);
    if (!isSecure)
    {
      ...
      if ("unsafeProcessHeader" in this.service)
        // No need to check that the connection is trustworthy,
        // the header comes from the user interface.
        // http://dxr.mozilla.org/mozilla-central/source/security/manager/boot/src/nsSiteSecurityService.cpp#407
        this.service.unsafeProcessHeader(this.service.HEADER_HSTS, uri, "max-age=" + maxAge, 0);
      else
